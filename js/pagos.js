...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function updateAuthUI(session) {
        if (session) {
            ...
            if (loggedInActions) loggedInActions.style.display = 'flex'; // Mostrar los botones de perfil y cerrar sesión en desktop

            // Actualizar el texto del botón de perfil si tienes el nombre de usuario
            const { data: { user } } = await supabase.auth.getUser();
            if (user) {
                const userName = user.user_metadata?.username || user.email;
                if (profileButton) profileButton.textContent = `Hola, ${userName.split('@')[0]}`;
                if (profileLinkMobile) profileLinkMobile.textContent = `Hola, ${userName.split('@')[0]}`; // También para el menú móvil
            }
            ...
            if (loggedInActionsMobile) loggedInActionsMobile.style.display = 'flex'; // Mostrar los enlaces de sesión iniciada
            if (loggedOutActionsMobile) loggedOutActionsMobile.style.display = 'none'; // Ocultar los enlaces de no sesión iniciada
            ...
            if (loggedInActions) loggedInActions.style.display = 'none'; // Ocultar los botones de perfil y cerrar sesión

            // --- Lógica para el menú móvil (enlaces dentro del desplegable) ---
            if (loggedInActionsMobile) loggedInActionsMobile.style.display = 'none'; // Ocultar los enlaces de sesión iniciada
            if (loggedOutActionsMobile) loggedOutActionsMobile.style.display = 'flex'; // Mostrar los enlaces de no sesión iniciada
